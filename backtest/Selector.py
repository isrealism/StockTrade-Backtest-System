...
def compute_atr(df: pd.DataFrame, n: int = 14) -> pd.Series:
    """
    计算 ATR (Average True Range) - 真实波幅指标
...
class BBIKDJSelector:
    ...
    def __init__(
        ...
        indicator_store: Optional[Any] = None,  # 新增：数据库查询接口
    ) -> None:
        ...
        self.indicator_store = indicator_store  # ← 新增

    # ---------- 单支股票过滤 ---------- #
    def _passes_filters(self, hist: pd.DataFrame) -> bool:
        """
        根据是否有 indicator_store 选择使用数据库模式或传统计算模式
        """
        if self.indicator_store is not None:
            return self._passes_filters_with_db(hist)
        else:
            return self._passes_filters_legacy(hist)

    def _passes_filters_with_db(self, hist: pd.DataFrame) -> bool:
        """
        使用数据库预计算指标的过滤逻辑（高性能模式）
    ...
    def _passes_filters_legacy(self, hist: pd.DataFrame) -> bool:
        """
        原有的实时计算逻辑（向后兼容）
        """
        hist = hist.copy()
...
class SuperB1Selector:
    ...
    def __init__(
        ...
        *,
        ...
        # ---------- 内部 BBIKDJSelector ----------
        # 如果有 indicator_store，也传递给内部选股器
        if indicator_store is not None and B1_params is not None:
            B1_params['indicator_store'] = indicator_store
...
class BBIShortLongSelector:
    ...
    def _passes_filters_with_db(self, hist: pd.DataFrame) -> bool:
        ...
        # 2. 计算短/长期 RSV（需要实时计算，因为窗口参数可配置）
        hist = hist.copy()
        
        # 尝试使用预计算的 RSV (如果参数匹配)
        if f'rsv_{self.n_short}' in hist.columns:
            hist["RSV_short"] = hist[f'rsv_{self.n_short}']
        else:
            hist["RSV_short"] = compute_rsv(hist, self.n_short)
            
        if f'rsv_{self.n_long}' in hist.columns:
            hist["RSV_long"] = hist[f'rsv_{self.n_long}']
        else:
