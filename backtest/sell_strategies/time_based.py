...
class EarlyExitStrategy(SellStrategy):
    ...
    逻辑
    ----
    买入后若连续 ``consecutive_days`` 个交易日，每日涨跌幅（当日收盘 vs 前日收盘）
    均处于 [daily_lower, daily_upper] 区间，认为股票处于横盘状态、缺乏启动动能，
    直接平仓释放资金。
    ...
    - daily_lower     = -0.03: 每天跌幅不超 3%（更深的回调交给止损策略处理）
    ...
    def __init__(
        ...
        daily_lower: float = -0.03,   # 每日涨幅下界（含），-3%
        ...
        **params,
        ...
        Args:
            ...
            daily_lower:      每日涨幅区间下界，小数（默认 -0.03 即 -3%）
            daily_upper:      每日涨幅区间上界，小数（默认 +0.03 即 +3%）
        """
        super().__init__(**params)
        self.consecutive_days = consecutive_days
        self.daily_lower = daily_lower
        self.daily_upper = daily_upper

        if self.daily_lower >= self.daily_upper:
            raise ValueError(
                f"daily_lower ({self.daily_lower}) 必须小于 daily_upper ({self.daily_upper})"
            )
    ...
    def should_sell(
        ...
        **kwargs,
        ...
        # ── 判断：全部日涨幅都在横盘区间内 ──────────────────────────
        all_sideways = bool(
            np.all(daily_returns >= self.daily_lower) and
        ...
        if all_sideways:
            ...
            return True, (
                ...
                f"[{returns_str}] "
                f"均在 [{self.daily_lower*100:.1f}%, {self.daily_upper*100:.1f}%] 内 "
    ...
    def get_name(self) -> str:
        return (
            ...
            f"[{self.daily_lower*100:.1f}%,{self.daily_upper*100:.1f}%])"
        )