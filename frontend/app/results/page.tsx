...
export default function ResultsPage() {
  ...
  const analysis = (analysisData?.analysis || result?.analysis) as
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">结果分析</h1>
          <p className="text-sm text-muted-foreground">
            回测结果仪表盘 - 策略表现分析与交易明细
          </p>
        </div>
        <div className="flex items-center gap-3">
          {/* ✅ 任务配置按键：仅在选中任务且 payload 存在时显示 */}
          {selectedId && backtestDetail?.payload && (
            <PayloadViewerDialog payload={backtestDetail.payload} />
          )}
          <BacktestSelector
